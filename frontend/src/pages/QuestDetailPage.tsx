...
export function QuestDetailPage() {
  ...
  // Load quest + restore draft for first task
  useEffect(() => {
    if (!questId) return
    getQuest(questId)
      .then(async (q) => {
        ...
        if (first) {
          try {
            const draft = await getDraft(q.id, first.id)
            setCode(draft.code || first.starter_code)
          } catch {
  ...
  // Save panel widths after drag ends
  const prevLeftDragging = useRef(false)
  const prevRightDragging = useRef(false)
  useEffect(() => {
    if (prevLeftDragging.current && !leftPanel.dragging) {
      savePreference('leftPanelWidth', String(leftPanel.width)).catch(() => {})
    }
    if (prevRightDragging.current && !rightPanel.dragging) {
      savePreference('rightPanelWidth', String(rightPanel.width)).catch(() => {})
    }
    prevLeftDragging.current = leftPanel.dragging
    prevRightDragging.current = rightPanel.dragging
  }, [leftPanel.dragging, leftPanel.width, rightPanel.dragging, rightPanel.width])

  // Auto-save draft (debounced 2s after code change)
  useEffect(() => {
    if (!quest || !activeTask) return
    if (saveTimer.current) clearTimeout(saveTimer.current)
    saveTimer.current = setTimeout(() => {
      saveDraft(quest.id, activeTask.id, code).catch(() => {})
    }, 2000)
    return () => {
      if (saveTimer.current) clearTimeout(saveTimer.current)
    }
  }, [code, quest, activeTask])

  // Switch task: save current draft, load new
  const switchTask = async (idx: number) => {
    // Save current draft before switching
    if (quest && activeTask && code !== activeTask.starter_code) {
      saveDraft(quest.id, activeTask.id, code).catch(() => {})
    }
    setActiveIndex(idx)
    setResult(null)
    ...
    if (task) {
      try {
        const draft = await getDraft(quest.id, task.id)
        setCode(draft.code || task.starter_code)
      } catch {
  ...
  const loadHistory = useCallback(async () => {
    if (!questId) return
    try {
      const data = await getHistory(questId)
      setHistory(data)
    } catch { /* ignore */ }
  }, [questId])

  useEffect(() => {
    if (rightTab === 'history') loadHistory()
  }, [rightTab, loadHistory])

  // Get a hint
  const showHint = () => {
    if (!activeTask || activeTask.hints.length === 0) return
    const hint = activeTask.hints[Math.floor(Math.random() * activeTask.hints.length)]
    toast(hint, { icon: '\u{1F4A1}', duration: 6000, style: { background: '#16213e', color: '#e2a03f' } })
  }
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          {/* Tab switcher */}
          <div className="flex shrink-0 border-b border-text-muted/20">
            <button
              onClick={() => setRightTab('result')}
              className={`flex-1 py-2 text-xs font-semibold transition-colors cursor-pointer ${
                rightTab === 'result'
                  ? 'text-gold border-b-2 border-gold bg-surface-panel/50'
            ...
            <button
              onClick={() => { setRightTab('history'); loadHistory(); }}
              className={`flex-1 py-2 text-xs font-semibold transition-colors cursor-pointer ${
                rightTab === 'history'
                  ? 'text-gold border-b-2 border-gold bg-surface-panel/50'
                  : 'text-text-muted hover:text-text-primary'
          ...
          <div className="flex-1 overflow-auto p-4">
            {rightTab === 'result' ? (
              ...
              /* History Tab */
              history.length === 0 ? (
                <p className="text-text-muted text-sm">暂无运行记录</p>
              ) : (
                  ...
                  {history.map((h) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className={h.passed ? 'text-success' : 'text-error'}>
                          {h.passed ? '✓' : '✗'} {h.task_id}
                        </span>
                      ...
                      <pre className="text-text-muted whitespace-pre-wrap line-clamp-2">
